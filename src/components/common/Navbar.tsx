...
export default function Navbar() {
  return (
    <nav className="py-6 md:mb-10 flex flex-row items-center justify-around md:justify-evenly">
      ...
      <ul className="hidden md:flex md:flex-row md:items-center md:justify-center md:gap-x-4 lg:gap-x-12">
        {socialLinks.map((link) => (
          <Link
            to={link.link}
            className="flex flex-row md:text-sm lg:text-lg md:gap-x-1 lg:gap-x-3 items-center md:hover:scale-110 md:transition-all md:border-2 lg:border-4 border-slate-500 dark:border-orange-500 md:p-2 lg:px-3 lg:py-2 md:rounded-2xl lg:rounded-3xl"
            target="_blank"
            key={link.link}
          >
            <link.icon />
